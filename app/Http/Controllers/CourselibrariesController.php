...
class CourselibrariesController extends Controller
{
    ...
    public function addLibrary(Request $request)
    {
        ...
        $courseProduct=Course::where('teaching_id','=',\Config::get('constants.paidOption'))->where('is_deleted','=','0')->get()->toArray();
        $CourseCategory=CourseCategory::where('teaching_id','=',\Config::get('constants.paidOption'))->where('status','=','1')->where('is_deleted','=','0')->whereNotIn('id',[2,5])->get();

    	return view('admin.course_library.add_library',['TeachingOtption'=>$TeachingOtption,'courseProduct'=>$courseProduct,'CourseCategory'=>$CourseCategory]);
    }
