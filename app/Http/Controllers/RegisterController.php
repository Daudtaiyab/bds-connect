    ...
    public function userRegister(Request $request)
    {
    	if($request->isMethod('post'))
    	{
    		...
    		$userExist=User::where('email','=',$input['email'])
                            ->orWhere('username','=',trim($input['username']))
                            ->orWhere('phone_number','=',trim($input['phone_number']))
    						->where('is_deleted','=','0')
    						->get()->toArray();
    		...
    		{ 
				...
				$userEmail=User::where('email','=',$input['email'])
                  ...
                  if(!empty($userEmail))
                  {
                    return back()->with('fail','User Email already exist')->withInput(); 
                  }    

                 $user_Username=User::where('username','=',trim($input['username']))
                            ->where('is_deleted','=','0')
                            ->get()->toArray();
                            
                 if(!empty($user_Username))
                 {
                    return back()->with('fail','Username already exist')->withInput(); 
                 }
                 
                 $Phone_Username=User::where('phone_number','=',trim($input['phone_number']))
                            ->where('is_deleted','=','0')
                            ->get()->toArray();
                if(!empty($Phone_Username))
                 {
                    return back()->with('fail','User Phone number already exist')->withInput(); 
                 }
    		}				
    		else
    		{
	    		...
	    		$userData->last_name=NULL;//$input['last_name'];
                ...
                $userData->wifi_address=$input['wifi_address'];
	    		$userData->status=0;
