...
class SchoolController extends Controller
{
    ...
    {	
    	...
    	$userList=User::select('users.id',DB::raw('CONCAT(users.first_name," ",users.last_name) as full_name'),'users.status','users.email','users.phone_number','users.username','users.address')
    ...
    {   
    	...
    	$userData=User::select('users.id','users.first_name','users.last_name','users.status','users.email','users.phone_number','users.username','users.status','users.address','users.wifi_address')
    					->where('users.id','=',$id)
    					//->where('users.status','=','1')
    					->where('users.is_deleted','=','0')
    					->whereIn('users.is_approved',[0,1])
    ...
    {
        $userList=User::select('users.id',DB::raw('CONCAT(users.first_name," ",users.last_name) as full_name'),'users.status','users.email','users.phone_number','users.username')
                        //->where('users.status','=','1')
                        ->where('users.is_deleted','=','0')
                        ->whereIn('user_role_id',[\Config::get('constants.is_user')])
                        ->whereIn('users.is_approved',[0,1]);
        ...
        $columns = array('S.No', 'Name','Email','Phone Number','User Name','Status','Course List');
