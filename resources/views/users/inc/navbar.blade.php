    ...
    <div class="dropdown options">
        ...
        <ul class="dropdown-menu" aria-labelledby="options">
            @if (Auth::user()->teach_id == \Config::get('constants.paidOption'))
                <li style="display: none;"><a class="dropdown-item" href="{{ route('spritesForm') }}"
                        onclick="event.preventDefault();
                                                     document.getElementById('sprites-form').submit();"><i
                            class="bi bi-toggles2"></i> Switch to Sprites</a></li>



                <li><a class="dropdown-item" href="{{ route('deviceForm') }}?switch=2"
                        onclick="event.preventDefault();
                                                     document.getElementById('device-form').submit();"><i
                            class="bi bi-ui-checks-grid"></i> My Courses</a></li>
                <li style="display: none;"><a class="dropdown-item" href="{{ route('MoreProduct') }}"
                        onclick="event.preventDefault();
                                                     document.getElementById('more-product').submit();"><i
                            class="bi bi-ui-checks"></i> More Products</a></li>
            @endif
            @if (Auth::user()->teach_id == \Config::get('constants.freeOption'))
                <li><a class="dropdown-item" href="{{ route('deviceForm') }}?switch=1"
                        onclick="event.preventDefault();
                                                     document.getElementById('device-form').submit();"><i
                            class="bi bi-ui-checks-grid"></i> Switch to Device</a></li>
            @endif
