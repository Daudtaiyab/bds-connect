    ...
    @if (isset($sidebarArr) && !empty($sidebarArr))
        @foreach ($sidebarArr as $listArr)
            <li class="cd-accordion__item cd-accordion__item--has-children">
                <input class="cd-accordion__input" type="checkbox" name ="group-{{ $listArr['cat_id'] }}"
                    id="group-{{ $listArr['cat_id'] }}" @if (in_array($listArr['cat_id'], [\Config::get('constants.paidPdf'), \Config::get('constants.freePdf')])) {{ 'checked' }} @endif>
                    ...
                    @if (isset($listArr['cat_name']) && !empty($listArr['cat_name']))
                        ...
                        @foreach ($listArr['cat_level'] as $key1 => $sub_level)
                            @if (!in_array($listArr['cat_id'], [\Config::get('constants.paidVideo'), \Config::get('constants.freeVideo')]))
                                <li class="cd-accordion__item cd-accordion__item--has-children">
                                    <input class="cd-accordion__input" type="checkbox"
                                        name ="sub-group-{{ $sub_level['level_id'] }}"
                                        id="sub-group-{{ $sub_level['level_id'] }}"
                                        @if (in_array($listArr['cat_id'], [\Config::get('constants.paidPdf'), \Config::get('constants.freePdf')]) && $iii == 0) {{ 'checked' }} @endif>
                                    <label class="cd-accordion__label cd-accordion__label--icon-folder"
                                        for="sub-group-{{ $sub_level['level_id'] }}"><span>{{ $sub_level['level_name'] }}</span></label>
                                    <ul class="cd-accordion__sub cd-accordion__sub--l2">
                                        @if (isset($sub_level['lesson_data']) && !empty($sub_level['lesson_data']))
                                            @foreach ($sub_level['lesson_data'] as $key => $less)
                                                <li class="cd-accordion__item"><a
                                                        class="cd-accordion__label cd-accordion__label--icon-img show_data"
                                                        href="#" id="{{ $less['lesson_id'] }}"
                                                        data-level="{{ $sub_level['level_id'] }}"><span>{{ $less['lesson_title'] }}</span></a>
                                                </li>
                                                <input type="hidden" name="next_prev[]"
                                                    value="{{ $less['lesson_id'] }}"
                                                    class="next_prev_{{ $sub_level['level_id'] }}"
                                                    id="next_prev_{{ $sub_level['level_id'] }}">
                                            @endforeach
                                        @endif
                            ...
                            @else
                                <li class="cd-accordion__item"><a
                                        class="cd-accordion__label cd-accordion__label--icon-img show_data"
                                        href="#" id="{{ $sub_level['lesson_id'] }}"
                                        data-level="raja"><span>{{ $sub_level['lesson_title'] }}</span></a></li>

                                <input type="hidden" name="next_prev[]" value="{{ $sub_level['lesson_id'] }}"
                                    class="next_prev_raja" id="next_prev_raja">
                            @endif
                            @php $iii++; @endphp
                        @endforeach
                    @endif
